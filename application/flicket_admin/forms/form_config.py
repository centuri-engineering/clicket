...
class ConfigForm(FlaskForm):
    ...
    mail_port = IntegerField(
        lazy_gettext("mail_port"), validators=[NumberRange(min=1, max=65535)]
    )
    ...
    mail_default_sender = StringField(
        lazy_gettext("mail_default_sender"), validators=[]
    )
    ...
    mail_ascii_attachments = BooleanField(
        lazy_gettext("mail_ascii_attachments"), validators=[]
    )

    application_title = StringField(
        lazy_gettext("application_title"),
        validators=[DataRequired(), Length(min=3, max=32)],
    )
    posts_per_page = IntegerField(
        lazy_gettext("posts_per_page"),
        validators=[DataRequired(), NumberRange(min=10, max=200)],
    )
    allowed_extensions = StringField(
        lazy_gettext("allowed_extensions"), validators=[DataRequired()]
    )
    ticket_upload_folder = StringField(
        lazy_gettext("ticket_upload_folder"), validators=[DataRequired()]
    )
...
class EmailTest(FlaskForm):
    email_address = StringField(
        lazy_gettext("email_address"),
        validators=[DataRequired(), check_email_formatting],
    )
