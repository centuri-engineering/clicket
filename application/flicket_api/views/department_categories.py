...
def get_department_categories():
    ...
    department_categories = FlicketDepartmentCategory.query.order_by(
        FlicketDepartmentCategory.department_category
    )
    kwargs = {}
    if department_category:
        department_categories = department_categories.filter(
            FlicketDepartmentCategory.department_category.ilike(
                f"%{department_category}%"
            )
        )
        kwargs["department_category"] = department_category
    if department_id:
        department_categories = department_categories.filter_by(
    ...
    data = FlicketDepartmentCategory.to_collection_dict(
        ...
        page,
        per_page,
        "bp_api.get_department_categories",
        **kwargs,
    )
    return jsonify(data)
