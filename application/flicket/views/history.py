...
@flicket_bp.route(
    app.config["FLICKET"] + "history/topic/<int:topic_id>/", methods=["GET", "POST"]
)
@login_required
def flicket_history_topic(topic_id):
    ...
    return render_template(
        "flicket_history.html", title=title, history=history, ticket=ticket
    )


@flicket_bp.route(
    app.config["FLICKET"] + "history/post/<int:post_id>/", methods=["GET", "POST"]
)
@login_required
