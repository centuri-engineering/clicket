...
def flicket_users(page=1):
    ...
    return render_template("flicket_users.html", title=title, users=users, form=form)


# view user details
@flicket_bp.route(
    app.config["FLICKET"] + "user/<int:user_id>/", methods=["GET", "POST"]
)
@login_required
def flicket_user(user_id):
    ...
    return render_template("flicket_user_details.html", title=title, user=user)
