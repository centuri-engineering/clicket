...
# tickets main
@flicket_bp.route(
    app.config["FLICKET"] + "ticket_department_category/<int:ticket_id>/",
    methods=["GET", "POST"],
)
@login_required
def ticket_department_category(ticket_id=False):
    ...
    if form.validate_on_submit():
        ...
        flash(
            gettext(f"You changed category of ticket: {ticket_id}"), category="success"
        )
        return redirect(url_for("flicket_bp.ticket_view", ticket_id=ticket.id))
    ...
    return render_template(
        "flicket_department_category.html", title=title, form=form, ticket=ticket
    )
