...
# tickets main
@flicket_bp.route(
    app.config["FLICKET"] + "ticket_institute/<int:ticket_id>/", methods=["GET", "POST"],
)
@login_required
def ticket_institute(ticket_id=False):
    ...
    if form.validate_on_submit():
        institute = FlicketDepartmentCategory.query.filter_by(
        ...
        if ticket.institute_id == institute.institute_id:
            flash(
                gettext(
                    f"Domain {ticket.institute.institute} is already assigned to ticket."
                ),
                category="warning",
            )
        ...
        # change category
        ticket.category_id = institute.category_id

        # add action record
        add_action(
            ticket,
            "institute",
            data={"institute": institute.institute, "institute_id": institute.institute_id,},
        )

        db.session.commit()

        flash(gettext(f"You changed institute of ticket: {ticket_id}"), category="success")
        return redirect(url_for("flicket_bp.ticket_view", ticket_id=ticket.id))

    title = gettext("Change Domain of Ticket")

    return render_template("flicket_institute.html", title=title, form=form, ticket=ticket)
