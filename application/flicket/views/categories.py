...
# create ticket
@flicket_bp.route(
    app.config["FLICKET"] + "categories/<int:department_id>/", methods=["GET", "POST"]
)
@login_required
def categories(department_id=False):
    ...
    if form.validate_on_submit():
        add_category = FlicketCategory(
            category=form.category.data, department=department
        )
...
@flicket_bp.route(
    app.config["FLICKET"] + "category_edit/<int:category_id>/", methods=["GET", "POST"]
)
@login_required
