...
from wtforms import PasswordField, StringField, FileField, SelectField
...
def check_password_formatting(form, field):
    ...
    if len(field.data) < min:
        ...
        ok = False
    if not any(s.isupper() for s in field.data) and not any(
        s.islower() for s in field.data
    ):
...
class CheckPasswordCorrect:
    ...
    def __call__(self, form, field):
        ...
        if user and not bcrypt.hashpw(self.password, user.password) == user.password:
            field.errors.append(
                "Your username and password do not match those in the database."
            )
            ok = False
...
class EditUserForm(FlaskForm):
    def __init__(self, *args, **kwargs):
        ...
        self.locale.choices = [
            (_id, lang) for _id, lang in app.config["SUPPORTED_LANGUAGES"].items()
        ]
    ...
    name = StringField(
        lazy_gettext("name"),
        validators=[
            Length(min=user_field_size["name_min"], max=user_field_size["name_max"])
        ],
    )
    email = StringField(
        lazy_gettext("email"),
        validators=[
            Length(min=user_field_size["email_min"], max=user_field_size["email_max"]),
            change_email,
        ],
    )
    ...
    new_password = PasswordField(
        lazy_gettext("new_password"),
        validators=[EqualTo("confirm", message="Passwords must match"),],
    )
    confirm = PasswordField(lazy_gettext("Repeat Password"))
    job_title = StringField(
        lazy_gettext("job_title"), validators=[Length(max=user_field_size["job_title"])]
    )
...
class ConfirmPassword(FlaskForm):
    password = PasswordField(
        lazy_gettext("password"), validators=[DataRequired(), check_password]
    )
