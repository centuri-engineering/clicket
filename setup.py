...
def run_set_up(self):
    ...
    create_default_requester_role_levels()
    create_default_request_stage_levels()
    create_default_procedure_stage_levels()
...
def create_default_request_stage_levels(silent=False):
    ...
    for level in request_stages:
        request_stage = FlicketRequestStage.query.filter_by(request_stage=level).first()
